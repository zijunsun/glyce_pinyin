...
from datasets.chn_sent_corp_dataset import ChnSentCorpDataset
...
class ChnSentiClassificationTask(pl.LightningModule):
    ...
    def get_dataloader(self, prefix="train") -> DataLoader:
        ...
        if self.args.task == 'ChnSentCorp':
            dataset = ChnSentCorpDataset(directory=self.args.data_dir, prefix=prefix,
                                         vocab_file=os.path.join(self.args.bert_path, "vocab.txt"),
                                         max_length=self.args.max_length,
                                         config_path=self.args.config_path)
        elif self.args.task == 'XNLI':
            dataset = XNLIDataset(directory=self.args.data_dir, prefix=prefix,
                                  vocab_file=os.path.join(self.args.bert_path, "vocab.txt"),
                                  max_length=self.args.max_length,
                                  config_path=self.args.config_path)
