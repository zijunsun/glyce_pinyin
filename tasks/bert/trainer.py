...
from datasets.bert_XNLI_dataset import BertXNLIDataset
from datasets.bert_chn_sent_corp_dataset import BertChnSentCorpDataset
...
class BertClassificationTask(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model = BertForSequenceClassification.from_pretrained(self.bert_dir)
    ...
    def get_dataloader(self, prefix="train") -> DataLoader:
        ...
        if self.args.task == 'ChnSentCorp':
            dataset = BertChnSentCorpDataset(directory=self.args.data_dir, prefix=prefix,
                                             vocab_file=os.path.join(self.args.bert_path, "vocab.txt"),
                                             max_length=self.args.max_length)
        elif self.args.task == 'XNLI':
            dataset = BertXNLIDataset(directory=self.args.data_dir, prefix=prefix,
                                      vocab_file=os.path.join(self.args.bert_path, "vocab.txt"),
                                      max_length=self.args.max_length)
...
def get_parser():
    ...
    parser.add_argument("--task", default="XNLI", type=str, help="train data path")
