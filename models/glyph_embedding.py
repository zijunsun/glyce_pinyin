...
def run_glyph_embedding():
    """test run"""
    import torch
    x = torch.LongTensor([[101, 3000, 800, 102], [101, 55, 56, 102]])
    bert_config_dir = "/data/nfsdata2/sunzijun/glyce/glyce/bert_chinese_base_large_vocab"
    tokenizer = BertTokenizer.from_pretrained(bert_config_dir)
    embedder = GlyphEmbedding(font_npy_files=[
        "/data/nfsdata2/sunzijun/glyce/glyce/config/方正古隶繁体.ttf24.npy",
        "/data/nfsdata2/sunzijun/glyce/glyce/config/STXINGKA.TTF24.npy",
        "/data/nfsdata2/sunzijun/glyce/glyce/config/STFANGSO.TTF24.npy",
    ])
    print(embedder)
    output = embedder(x)
    print(output.shape)
